...
Run locally with `npm install`, then `npm run dev`. The public home is `/`; the authenticated app is `/app.html`; the all-pages preview is `/preview-all-pages.html`; the setup guide is `/setup-guide.html`. The preview and setup guide are local documentation artifacts and are not included in production Vite inputs. Validate with `npm run typecheck` and `npm run build`.
